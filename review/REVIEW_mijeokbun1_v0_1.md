...
- 템플릿 34개 (생성 대상 33 + needs_figure 보류 1)
...
- 원본 카드: `M1-DIF-09` / 단원: 미분 / 난이도: 2 / **needs_figure: 생성 보류**
- **curriculum_check**: 도함수 그래프의 부호에서 원함수의 극대·극소를 읽는 것은 미적분Ⅰ '도함수의 활용' 내용이다. 그래프 제시가 본질이므로 v0.1에서는 needs_figure로 생성 보류.
- 파라미터:
  - `r`: int, 범위 [-3, 1], 제외 {0}
  - `s`: int, 범위 [-1, 4], 제외 {0}
- 제약: `r < s` · `r + s != 0`
...
> stem 초안: $\text{[그림] 삼차함수 } y = f(x) \text{ 의 도함수 } y = f'(x) \text{ 의 그래프는 아래로 볼록한 포물선이고 } x = {r}, \; x = {s} \text{ 에서 } x \text{축과 만난다. } f(x) \text{ 가 극대가 되는 } x \text{ 의 값은?}$
...
- 원본 카드: `M1-INT-07` / 단원: 적분 / 난이도: 2
- **curriculum_check**: f(x)≤0인 구간에서 넓이 = -∫f dx 처리는 미적분Ⅰ '넓이' 기본 유형이며, x(x-a)는 [0,a]에서 항상 0 이하이므로 구간 분할이 필요 없는 형태로 설계했다.
- 파라미터:
  - `a`: int, 범위 [2, 6]
- 오답 설계 (mistake_type):
...
- 원본 카드: `M1-INT-08` / 단원: 적분 / 난이도: 2
- **curriculum_check**: 구간에서 (위 함수 - 아래 함수)의 정적분으로 두 곡선 사이 넓이를 구하는 것은 미적분Ⅰ '두 곡선 사이의 넓이' 유형이다. 직선 계수를 교점 r, s로부터 역산해 교점이 항상 정수가 되도록 설계했다.
- 파라미터:
  - `r`: int, 범위 [-2, 2], 제외 {0}
  - `s`: int, 범위 [-1, 3], 제외 {0}
- 제약: `r < s` · `r + s != 0`
...
- 원본 카드: `M1-INT-09` / 단원: 적분 / 난이도: 3
- **curriculum_check**: 이동 거리 = ∫|v(t)|dt 를 v=0인 시각 기준으로 구간을 나눠 계산하는 것은 미적분Ⅰ '속도와 거리' 대표 유형이다. b를 짝수로 제한해 답이 항상 정수가 된다.
- 파라미터:
  - `b`: choice, 선택지 {-2, -4, -6}
  - `p`: int, 범위 [2, 6]
- 제약: `p > -b/2`
...
> $f(x) = \begin{cases} x - 2 & (x < 1) \\ -2x + 4 & (x \ge 1) \end{cases} \text{ 일 때, } \lim_{x \to 1^{-}} f(x) - \lim_{x \to 1^{+}} f(x) \text{ 의 값은?}$
>
> ① $3$
> ② $-3$ **← 정답**
> ③ $0$
...
> ⑤ $-1$
>
> 파라미터: `{"a":-2,"b":-2,"c":4,"p":1}`

### 샘플 2

> $f(x) = \begin{cases} x - 2 & (x < -3) \\ -3x + 5 & (x \ge -3) \end{cases} \text{ 일 때, } \lim_{x \to -3^{-}} f(x) - \lim_{x \to -3^{+}} f(x) \text{ 의 값은?}$
>
> ① $-19$ **← 정답**
> ② $9$
> ③ $-5$
...
> $\lim_{x \to -2} \left( x^2 + 3x + 5 \right) \text{ 의 값은?}$
>
> ① $5$
> ② $-2$
> ③ $-3$
...
> $\lim_{x \to 1} \left( x^2 + 3x + 3 \right) \text{ 의 값은?}$
>
> ① $7$ **← 정답**
> ② $3$
> ③ $1$
...
> $\lim_{x \to -3} \dfrac{x^2 - 9}{x + 3} \text{ 의 값은?}$
>
> ① $-3$
> ② $6$
> ③ $9$
...
> $\lim_{x \to -4} \dfrac{x^2 + x - 12}{x + 4} \text{ 의 값은?}$
>
> ① $-7$ **← 정답**
> ② $-3$
> ③ $-1$
...
> $f(x) = \dfrac{-3\left| x - 0 \right|}{x - 0} - 4 \text{ 일 때, } \lim_{x \to 0^{-}} f(x) \text{ 의 값은?}$
>
> ① $-7$
> ② $-6$
> ③ $-4$
...
> $f(x) = \dfrac{-4\left| x - 0 \right|}{x - 0} - 1 \text{ 일 때, } \lim_{x \to 0^{-}} f(x) \text{ 의 값은?}$
>
> ① $-1$
> ② $3$ **← 정답**
> ③ $-8$
...
> $f(x) = \begin{cases} \dfrac{x^2 + x - 2}{x - 1} & (x \ne 1) \\ k & (x = 1) \end{cases} \text{ 가 } x = 1 \text{ 에서 연속일 때, 상수 } k \text{ 의 값은?}$
>
> ① $-3$
> ② $3$ **← 정답**
> ③ $1$
...
> $f(x) = \begin{cases} -2x + b & (x < 2) \\ x^2 + 5 & (x \ge 2) \end{cases} \text{ 가 } x = 2 \text{ 에서 연속일 때, 상수 } b \text{ 의 값은?}$
>
> ① $5$
> ② $3$
> ③ $9$
...
> $f(x) = \begin{cases} 3x + b & (x < -2) \\ x^2 - 6 & (x \ge -2) \end{cases} \text{ 가 } x = -2 \text{ 에서 연속일 때, 상수 } b \text{ 의 값은?}$
>
> ① $4$ **← 정답**
> ② $-4$
> ③ $-8$
...
> $\text{연속함수 } f(x) \text{ 가 } f(0) = -5,\; f(1) = 2,\; f(2) = -4,\; f(3) = 5 \text{ 를 만족시킬 때, 세 열린구간 } (0,1),\; (1,2),\; (2,3) \text{ 중에서 방정식 } f(x) = 0 \text{ 이 반드시 실근을 갖는다고 말할 수 있는 구간의 개수는?}$
>
> ① $2$
> ② $4$
> ③ $3$ **← 정답**
...
> ⑤ $0$
>
> 파라미터: `{"A":-5,"B":2,"C":-4,"D":5}`

### 샘플 2

> $\text{연속함수 } f(x) \text{ 가 } f(0) = 1,\; f(1) = -4,\; f(2) = 5,\; f(3) = -6 \text{ 를 만족시킬 때, 세 열린구간 } (0,1),\; (1,2),\; (2,3) \text{ 중에서 방정식 } f(x) = 0 \text{ 이 반드시 실근을 갖는다고 말할 수 있는 구간의 개수는?}$
>
> ① $0$
> ② $2$
> ③ $3$ **← 정답**
...
## needs_figure 템플릿 목록 (v0.1 생성 보류)

- **T-M1-DIF-09-a** (M1-DIF-09, 삼차함수 그래프와 도함수 - 도함수 그래프에서 극대점 읽기) — 그래프/그림 제시가 문제의 본질이라 텍스트만으로는 출제 불가. v0.2에서 그림 생성과 함께 활성화 예정.
