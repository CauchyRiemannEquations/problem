...
- 미적분Ⅰ 34장 (`mijeokbun1_ai_rag_cards_v0_1.jsonl`) → 템플릿 70개 (전부 생성 가능, 카드당 2~3유형)
  - 서로 다른 문제(파라미터 조합 기준) 총 약 69,700개
  - 카드마다 "숫자만 다른" 변형이 아니라 질문 방식이 다른 -b/-c 유형을 병행
    (역산형, 그래프 읽기형, 개수 세기형, 활용형 등)
  - 그림이 필요한 유형은 `figure` 스펙으로 SVG를 자동 생성 (9개):
    도함수 그래프 2종(DIF-09-a/b), 극한 그래프 읽기 2종(LIM-01-b, LIM-02-b),
    넓이 빗금 영역 4종(INT-07-a/b, INT-08-a/b), 속도 그래프 1종(INT-09-a)
...
- Releases에서 **BamsamFactory-Setup-*.exe** 를 받아 설치하면 자체 창을 가진 앱으로 실행된다
  (시작 메뉴·작업표시줄 고정 가능, 자체 아이콘).
- 실행 중 새 버전이 올라오면 자동으로 내려받아 "업데이트 준비됨" 알림을 띄우고,
  재시작하면 적용된다 (GitHub Releases 기반 electron-updater).
- 템플릿을 수정해 main에 푸시하면 CI가 새 버전(0.2.빌드번호)을 자동 발행 → 설치된 앱이 스스로 업데이트를 감지한다.
